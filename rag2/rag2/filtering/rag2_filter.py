...
identical logits at far lower cost. That is what ``score_pairs`` does.

tests/test_filter_first_token_logits.py pins the part of that equivalence this
code owns -- both branches read position 0, slice the same two label columns and
reach the same decision. That HuggingFace's ``generate().scores[0]`` *is* that
forward pass follows from the decoding semantics but is a property of
``transformers``, not of this repository, and is not asserted here;
``options.use_generate: true`` restores the literal generate() path if it is
ever in doubt.
