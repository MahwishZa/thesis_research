...
pytest                       # last measured: 194 passed, 2 skipped (torch-gated)
...
- **This repository's corpus** — `rag2/corpora/thesis_chunks.py` reads the chunk
  layer and MedCPT index built under `../pmc/` (`chunks.jsonl` plus
  `embeddings.f32` + `index_manifest.jsonl`). Configured by
  [`configs/thesis_corpus.yaml`](configs/thesis_corpus.yaml); one corpus instance
  per `source_category`, so balanced retrieval works over three corpora here
  rather than the paper's four. Run the stage scripts from the repository root so
