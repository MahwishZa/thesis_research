...
* The index manifest deliberately carries no chunk text, so the loader joins
  text back in from `chunks.jsonl` on `chunk_id`, and refuses to start if the
  two files disagree.
...
avoids a generate() call per snippet.

`tests/test_filter_first_token_logits.py` pins the half of that equivalence this
repository owns: both branches read decoder position 0, slice the same two label
columns, and produce the same keep/drop decision. The other half — that
`generate(...).scores[0]` *is* that forward pass — follows from the decoding
semantics (nothing has been decoded yet, so the position depends only on the
encoder output and the decoder start token) but is a property of `transformers`
and is **not** asserted here. `filter.options.use_generate: true` runs the
literal release path if it is ever in doubt. Both tests require torch and skip
without it.
