...
## Sources
Defined in `config/sources.yaml`: PubMed (E-utilities), PMC Open Access, clinical guidelines,
currency pack. Textbooks are **disabled by default** and metadata-only.
...
## Preprocessing
`raw → parsed → normalized → deduplicated → chunks`, one directory per stage under `data/`. Deduplication precedes chunking.
...
## Source tiers
Nine tier values in `config/sources.yaml`, **deliberately unordered**. Authority ordering is a tested thesis variable
...
## Reproduction
```bash
python3 alzheimers_corpus/src/retrieval/build_queries.py --check
cd alzheimers_corpus/tests && python3 -m unittest test_ad_relevance -v
