...
**Expected output.** Raw responses under `alzheimers_corpus/data/raw/`, manifests under
`alzheimers_corpus/data/manifests/`.
...
```bash
python3 alzheimers_corpus/src/retrieval/build_queries.py --check   # queries match config
...
`alzheimers_corpus/queries/pubmed/Q0*.txt`. Commit those counts — they are provenance.
...
`alzheimers_corpus/metadata/guideline_registry.csv`.
...
**What you must do.** Populate `alzheimers_corpus/metadata/textbook_registry.csv` with metadata
...
`alzheimers_corpus/metadata/currency_pack_registry.csv` with its verified licence.
...
`alzheimers_corpus/annotations/claim_validation_300.csv`. The sample must be **stratified** across
