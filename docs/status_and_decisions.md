...
| **Filter training** | a *trained* RAG² baseline | strategy decided, label-generation and training code now implemented (`experiments/filter_training/{medqa_data,rationale,build_labels,train}.py`, 2026-09-21i) — untested end-to-end (no GPU/torch in this environment); needs one Colab GPU session to actually run and produce a checkpoint |
