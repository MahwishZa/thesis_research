...
A companion document, [`rag2_reproduction_audit.md`](rag2_reproduction_audit.md),
audits the implementation against this specification and records the defects
found and corrected.
...
* **[S] Which generation of it?** The paper has *one* rationale per question:
  generated by the base LLM, used as the retrieval query (§3.3) and scored for
  perplexity (Figure 2). Labeling therefore **reuses the rationale the cache
  stored at stage 1** rather than generating a second one, so the passage a
  rationale retrieved is judged by that same rationale. Config key:
  `filter_training.rationale_source: cached | regenerate`. `cached` falls back
  to regeneration — recording why in the diagnostics — when no rationale is
  cached, or when the closed-book answer prompt has been overridden so the
  cached text is a completion of a different prompt.
* Prompt tokens are masked out of the loss; only rationale tokens contribute.
* Length normalisation is by rationale token count `L`, per Equation 4. Both
  terms score the same `L`: if a continuation cannot fit the context window,
  scoring raises rather than silently truncating it (which would make the two
  terms of Equation 3 incomparable).
...
* **[A]** Non-finite ΔPPL — from a degenerate rationale (empty generation, or a
  scoring failure) — is excluded from both τ and labeling, and counted in
  `num_non_finite_excluded`. The paper does not contemplate an undefined
  differential; labeling one would let IEEE comparison semantics (`inf ≥ τ` is
  true, `nan ≥ τ` is false) decide a training label.
