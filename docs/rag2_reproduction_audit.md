...
| | |
| --- | --- |
| Audited commit | `746b611` (+ the three fixes in [§5](#5-defects-found-and-fixed)) |
| Automated checks | 42, reproducible with `python -m rag2_audit.run` |
| Result | 31 PASS · 0 FAIL · 5 PARTIAL · 3 UNKNOWN · 2 APPROXIMATION · 1 MANUAL |
| Machine-readable | `audit/audit_results.json` |
| End-to-end trace | `audit/trace_synthetic.json` (`python -m rag2_audit.trace`) |
| Quantitative tests | `tests/test_audit_quantitative.py` (25), `tests/test_audit_module.py` (12) |
...
| Configuration hygiene | PARTIAL | STR-04: 10 dead keys → [§4.5](#45-str-04--ten-dead-configuration-keys) |
...
| Which rationale is scored | PARTIAL | LBL-05 → [§4.2](#42-lbl-05--the-scored-rationale-is-regenerated-not-the-one-retrieval-used) |
...
| Answer-generation prompt | UNKNOWN | GEN-02 → [§4.4](#44-gen-02--the-answer-generation-prompt-was-never-published) |
| Context = kept passages in rank order | PASS | GEN-03 |
| Generation length keys | PARTIAL | GEN-04 → [§4.5](#45-str-04--ten-dead-configuration-keys) |
| Accuracy metric | PASS | EVA-01 |
| Answer extraction | UNKNOWN | EVA-02 → [§4.4](#44-gen-02--the-answer-generation-prompt-was-never-published) |
...
### 4.2 LBL-05 — the scored rationale is regenerated, not the one retrieval used

**Paper.** §3.3 describes **one** rationale per question: generated by the base
LLM, used as the retrieval query. Figure 2 scores "Rationale" from the same LLM
output. There is no second rationale in the paper.

**Implementation.** `build_labels.py` calls the LLM again with the closed-book
prompt and scores *that* generation, discarding `CandidateSet.rationale` written
by stage 1. Confirmed: the cached string is not what gets scored.

**Why it matters.** The two agree only while decoding is exactly deterministic
*and* the same backbone and prompt are configured for both stages. The paper
itself reports residual nondeterminism at temperature 0 (A.3), and nothing
asserts that the cache's rationale LLM matches the labeling LLM — so the query
that retrieved a passage can silently differ from the rationale whose perplexity
judges it. It also doubles the closed-book generation cost of the most expensive
stage.

**Correction.** Reuse `CandidateSet.rationale` when present, generating only the
correctness signal; or assert the regenerated rationale equals the cached one and
fail loudly otherwise. **Not applied** — it changes what gets scored, which is a
research decision rather than an unambiguous bug fix. Recommended before the
labeling run.
...
### 4.5 STR-04 / GEN-04 — dead and ambiguously-scoped configuration

**Implementation.** Ten declared config keys are never read:
`ExperimentConfig.notes`, `CorpusConfig.chunk_size`, `CorpusConfig.chunk_overlap`,
`RetrievalConfig.article_encoder`, `RetrievalConfig.shard_size`,
`GenerationConfig.stop`, `EvaluationConfig.metric`,
`EvaluationConfig.open_ended_metrics`, `EvaluationConfig.bertscore_model`,
`CacheConfig.enabled`. Separately, rationale length is governed by
`llm.max_new_tokens` while answer length is governed by
`generation.max_new_tokens` — same name, same default, different stages.

**Why it matters.** A key that looks like a control but is a no-op invites a
silent deviation: someone sets `generation.stop` or `cache.enabled` and nothing
happens. The length keys are worse: setting `generation.max_new_tokens` to study
answer length leaves the rationale — and therefore the retrieval query *and every
perplexity label* — unchanged.

**Correction.** Wire each key or delete it (keeping the value in the manifest);
rename the length keys to name their stage. Housekeeping, not a method error, so
**not applied** — it is a behaviour-neutral change best made deliberately.
...
**Deliberately not changed:** LBL-05 (§4.2) and STR-04/GEN-04 (§4.5). The first
changes what gets scored; the second is behaviour-neutral housekeeping. Both are
the user's call, per the brief's instruction not to modify the implementation
absent a clear reproduction bug.
