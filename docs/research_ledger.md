...
**Last updated:** 2026-09-18 · **Current pipeline stage:** Step 1 PMC source EXECUTED (D-42); other Step 1 sources and normalize/dedup/chunk in progress; Step 2 (questions) in progress; Steps 3–12 infrastructure built, unexecuted · **Status:** Not executable yet
...
| D-41 | **Stage 02 PMC finalization failed on a producer/consumer evidence mismatch, not on corpus damage. The reader was widened to the wordings actually written; no corpus file was altered** | Finalization treats `retrieval.log` as the authoritative record of which PMCIDs PMC holds no article version for. Its regex required `"PMC123: no article version found; recording unavailable_current_dataset"`, but the corpus was retrieved on 2026-09-13/15 by an earlier version of the script that wrote only `"PMC123: no article version found"`, and a later targeted retry wrote `"... during targeted retry"`. All three assert the same finding. The reader matched none of the historical lines, recovered **0** unavailable PMCIDs from a log containing **102**, and therefore could not account for them — so it refused to write the manifest on every attempt (2026-09-18 02:44 and 04:43, both ended by the operator). No rerun could ever fix this: log lines already written cannot be reworded. The fix matches the core statement and ignores the trailing prose, which widens what can be *read* without weakening what counts as evidence. Verified against the committed log: 102 unavailable recovered, repair evidence still 693 non-OA + 9 stale, accounting closes at 112,260 | **Adopted** |

| D-42 | **Stage 02 PMC finalization executed and independently verified 2026-09-18: 114,256-row manifest committed to `metadata/pmc.csv`** | After D-41's fix, the student re-ran `--finalize` to completion and pushed the result. Verified independently against the committed file rather than the run's own log line: row count 114,256 (0 duplicate pmcid/version pairs), 114,157 `already_verified` + 99 `unavailable_current_dataset` (sums to the total), every row's path fields consistent with its status, and three cross-checks against numbers reported before the fix was even designed — predicted row count, local XML file count (114,157), local JSON file count (114,850, since 693 non-OA versions have JSON but no XML) — all closed exactly. This is the repository's first EXECUTED artifact. It does not close Step 1: PubMed/guidelines/textbooks metadata and the normalize→deduplicate→chunk stages are unchanged and still hold only the 10-record synthetic fixture | **Adopted** |

| D-43 | **Stages 04-07 rewritten to consume the real corpus; 01/02/03 deliberately left untouched.** 04_normalize.py now reads metadata/pmc.csv + JATS XML by default (a new _common.py bridge: parse_jats_xml, iter_pmc_records), since Stage 01 never produced abstracts and nothing previously connected Stage 02's real output to Stage 04's expected shape. 07_claim_classification.py's load_classes() read cfg["groups"], which no longer exists in claim_taxonomy.yaml (restructured to claim_types/evidence_levels at some point after the committed claim_class_distribution.csv was produced under the old schema) - it crashed with KeyError on the current config and is rewritten against it | The audit that surfaced these (see next_steps.md) found 04-07 could only ever process the 10-record synthetic fixture; the real, already-executed PMC manifest and real taxonomy config were structurally incompatible with what the scripts read. 01_pubmed_download.py and 02_pmc_download.py were excluded from this rewrite on purpose: both are mature, already-executed code (676 real PMIDs, the independently-verified 114,256-row PMC manifest, ledger D-41/D-42) and "placeholder logic" does not describe them; rewriting either risks the real, already-collected data disagreeing with a "corrected" script. A related real bug found while testing the new path against realistic (not fixture) data: redistribution_allowed() matched only hyphenated licence codes ("CC-BY"); PMC's own license_code field is space-delimited ("CC BY"), so every real PMC record would have been mismarked non-redistributable regardless of its actual licence. Fixed by normalising both spellings before comparison; the set of licence families treated as distributable is unchanged | **Adopted** |
| D-44 | **claim_status, temporal_status and disease_relevance are deliberately NOT tagged by Stage 07** | The rewritten taxonomy (claim_taxonomy.yaml) documents these as three additional dimensions beyond claim_types/evidence_levels. claim_status and temporal_status both require comparing a passage against other evidence (is it contradicted? superseded?) that a single passage's keywords cannot establish; temporal status specifically is the thesis's actual experimental treatment (systems/proposed/recency.py scores it from publication date at admission time per docs/frozen_scope.md), so pre-baking a weaker keyword-based "current vs superseded" label onto the corpus would duplicate that treatment with a worse method and risk disagreeing with it. disease_relevance is already decided, with a full rule trace, by Stage 04's assess_ad_relevance() over each source document; a second, coarser chunk-level classifier for the same concept would risk an inconsistent second opinion rather than adding information, so Stage 07 propagates ad_relevant/ad_relevance_score (Stage 06 was extended to carry them onto each chunk, since it previously dropped them) instead of recomputing | **Adopted** |
...
| Date | Change |
|---|---|
| 2026-09-18 | Repository-wide audit. Restored `alzheimer_corpus/data/raw/pubmed/records.example.jsonl`, byte-identical to its pre-deletion content: a prior commit deleted it while removing `data/raw/pmc/.gitkeep`, but the corpus `.gitignore` still forces it tracked and `04_normalize.py` still defaults to it, so a fresh clone could no longer run stage 04 offline as documented. Fixed `pyproject.toml`'s stale package list (missing `experiments.retrieval`, `experiments.filter_training` — `pip install -e .` would not have installed them). Removed seven dead imports/variables found by `pyflakes` outside `alzheimer_corpus/scripts/` (left untouched there per the corpus-protection policy; none were bugs). Removed a genuinely redundant `.gitkeep` (`experiments/outputs/`, whose directory already has real tracked content). Marked `systems/interfaces/retriever.py` superseded by `experiments/retrieval/` in its own docstring rather than deleting it — zero callers, zero direct tests, but kept for backward compatibility per the repository's own no-cosmetic-deletion convention. Corrected stale "Stage N" terminology to "Step N" in three source docstrings and one doc, added superseded-plan notes to two point-in-time hardware/feasibility audits whose FAISS-based retrieval description D-37 superseded, and updated two stale status claims (README.md's "nothing has been executed", one test-count figure). 401 tests pass throughout; no corpus data, log, or metadata file touched. |
