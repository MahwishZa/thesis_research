...
| D-38 | **The generator is Qwen2.5-7B-Instruct at 4-bit NF4, executed on free-tier remote GPU (Colab/Kaggle T4 16 GB), with a pinned execution contract** | Llama-3-8B-Instruct is RAG²'s generator but is gated on Hugging Face and does not fit 4 GB VRAM; 4-bit weights alone are ≈5 GB before KV cache, so local GPU execution is not available at any quantization and CPU execution of an 8B model over ~100 questions × 2 arms is too slow to run twice for reproducibility. Qwen2.5-7B-Instruct is ungated, same order of capability and parameter count, and fits a free T4 with headroom. The substitution is a stated adaptation, not a silent swap; the requirement the thesis actually depends on is that **both arms use one generator instance under identical decoding**, which the contract and `assert_generator_parity` enforce | **Adopted** |
| D-39 | **The RAG² filter is trained by us on Flan-T5-base with LoRA, on free-tier remote GPU, from labels regenerated by the RAG² decision-tree procedure over a general-medical split** | The checkpoint is not distributed (E9) and the paper's Flan-T5-large (E1) label-generation cost is millions of forward passes (K12). Flan-T5-base (250M) trains inside a free session and is the smallest faithful instance of the same architecture and label function. This is a **thesis-specific adaptation of the reference method** and is documented as one: the filter is weaker than the paper's, which is a limitation on the baseline's strength, not a threat to the comparison, because the same filter defines the baseline for both HAR and accuracy and the proposed system is compared against it, not against the paper's numbers | **Adopted** |
