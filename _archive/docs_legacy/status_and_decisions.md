...
9. The corpus is an Alzheimer's domain slice, not RAG²'s 564 GB
   ...
   declared adaptation.
10. **515 of 111,315 corpus documents (~0.46%) produced a duplicate
    `chunk_id`** — two distinct renderings of the same paper that survived
    Stage 05's near-duplicate detection under a shared `document_id` (see
    the 2026-09-21g change-log entry for the full diagnosis). Handled at
    load time, not by editing the frozen corpus: the first occurrence in
    file order is kept, every drop is logged to `duplicate_chunks.json`,
    and this is disclosed rather than silently absorbed into "4,377,041
    chunks."
...
| 2026-09-21j | **Improved the error message for "cuda requested but torch has no CUDA support", hit for real on the student's first Kaggle attempt at Step 4.** The raw failure (`AssertionError: Torch not compiled with CUDA enabled`, eight frames deep in `torch.nn.Module._apply`) gave no indication of cause, and surfaced only after minutes of downloading/tokenizing. Root cause was environmental (a `pip install` on Kaggle silently replaced the preinstalled CUDA-enabled torch with a CPU-only build — a known Colab/Kaggle gotcha, not a bug in this repository), but the unhelpful error was real and worth fixing regardless. `experiments/retrieval/encoders.py` gained `_move_to_device()`, checking `torch.cuda.is_available()` before `.to(device)` and raising a specific, actionable `RuntimeError` naming the likely cause and the fix; `FlanT5RAG2Filter.__init__` (`systems/baseline/admission.py`) got the equivalent check inline. Fails in under a second instead of after downloading a model. 524 tests pass (same 1 pre-existing, unrelated failure); the new check itself could not be exercised in this environment (no torch installed here, by design). |
| 2026-09-21i | **Implemented the missing half of Step 4: the actual RAG² filter-training pipeline (label generation + training loop), to run on Colab.** Previously `experiments/filter_training/` had the label decision tree (`labeling.py`) and hyperparameter config (`config.py`) but no code that actually generates labels or trains anything — this closes that gap. **Two new, recorded design decisions**, since the official RAG² release does not pin either: (1) MedQA source is `GBaker/MedQA-USMLE-4-options` (public HF dataset, 4-option USMLE-style, matching the option format `admission.py` already documents) — the official repo's own `classifier/data/medqa/llama3_cot/5%-train.json` is a 5-example format illustration, not a usable split, and names no source dataset id. (2) Evidence passages for label generation are retrieved from `MedRAG/textbooks` (public HF dataset, pre-chunked medical textbook passages) rather than the thesis's own Alzheimer's corpus — using the AD corpus would make the "general-medical" filter secretly AD-specific, defeating the entire reason (specification SS10.6) it must be trained outside the thesis's own data. This is recorded as a reasoned substitute for RAG²'s own 564GB corpus (not available here), not a verified match to it. **New files**: `medqa_data.py` (loaders + `extract_answer_letter`, with defensive schema checks that fail loudly rather than silently misreading a changed dataset), `rationale.py` (`Llama3RationaleScorer` — implements `labeling.RationaleScorer` using Llama-3-8B-Instruct with chain-of-thought prompting, matching the paper's own `llama3_cot` label-generation path; computes rationale perplexity via a teacher-forced forward pass, since `HuggingFaceGenerator.generate()` does not expose logits), `build_labels.py` (CLI: retrieve one passage per MedQA question from a textbook index, score with/without evidence, label via the existing decision tree, write the training file), `train.py` (CLI: add `[HELPFUL]`/`[NOT_HELPFUL]` as special tokens, resize embeddings, fine-tune with `FilterTrainingConfig`'s documented hyperparameters, write a `CheckpointRecord`). `train.py`'s data split is explicitly named and documented as unrelated to the thesis's own Alzheimer's validation/test split (`experiments/questions/splits.json`) — same word "split", different purpose (fitting a classifier's weights vs. fitting λ/θ/H), kept apart on purpose. **Honesty note**: `rationale.py` and `train.py`'s model-loading/training code could not be executed or verified in this environment (no torch/transformers/datasets/GPU here) — written by careful code review against the documented spec, not tested end-to-end. The pure-Python logic (`MedQAItem` validation, `extract_answer_letter`, `train.py`'s train/val split) was extracted, tested, and verified directly (14 new tests, `tests/unit/test_filter_training_data.py`). `compute_metrics`'s single-token-accuracy assumption is flagged in-code as needing a small-scale sanity check before being trusted at full training scale. 524 tests pass; the same 1 pre-existing, unrelated failure. |
| 2026-09-21h | **Step 4 readiness audit (RAG² checkpoint + Llama-3-8B-Instruct), while the real index build ran in the background.** Confirmed by inspection and by actually running it: `experiments/runners/run_end_to_end.py`'s fixture path (`make_fixture_items`, `make_generator`, `make_rag2_filter`, `build_systems`) already wires retriever/generator/filter interfaces correctly and executes a full question→retrieval→generation→scoring path with 0 errors, without touching the real corpus or a GPU — this already is the minimal e2e readiness check; no real one existed to duplicate. **No RAG² checkpoint exists anywhere in the repository** (confirmed by search) — none was fabricated; `FlanT5RAG2Filter` correctly refuses a stock checkpoint (requires `[HELPFUL]`/`[NOT_HELPFUL]` as single added tokens) and `make_rag2_filter()` correctly falls back to the documented `MockRAG2Filter` stand-in with clear report labelling when none is given. **One genuine small gap fixed**: neither `FlanT5RAG2Filter` nor `run_end_to_end.py`'s CLI could be told to use CPU instead of auto-picking CUDA, which matters now that a GPU is mid-use by the index build — added `--rag2-device` (default: unchanged auto-pick behaviour) so a future checkpoint can be sanity-checked on CPU without touching whatever else is using the GPU. Verified end-to-end again after the change (22 integration tests, fixture run repeated manually). **Llama-3-8B-Instruct**: license confirmed already granted (2026-09-21); loading code (`HuggingFaceGenerator`/`ModelSpec`) already implemented and already confirmed working against a real model once before (TinyLlama stand-in, 2026-09-19) — not re-attempted now, since it needs ~16GB of download and real GPU memory that would contend with the running index build; per the existing hardware assessment (`status_and_decisions.md` §5, §9) it must run on a remote GPU session regardless, never the local 4GB card. **Also applied two safe, zero-behavior-change speed fixes found while auditing the encoder for the concurrent index-speed question** (see the same-day entry below): `torch.no_grad()` → `torch.inference_mode()` in `MedCPTEncoder`/`MedCPTReranker` (strictly faster, identical output), and explicit `use_fast=True` on tokenizer loading (defensive; guards against a slow pure-Python tokenizer silently being selected). These do not touch the running process — a running Python process already has last-9-18's code loaded in memory; editing the file does not affect it. 510 tests pass; the same 1 pre-existing, unrelated failure. |
| 2026-09-21g | **Diagnosed and handled the real corpus's duplicate `chunk_id`s, found when the student's first real `build_index.py` run hit the uniqueness check.** Diagnosis (read-only, against the real corpus, on the student's machine — not assumed): 515 duplicated `chunk_id`s / 900 extra lines out of 4,377,041 (~0.02%). A structural diff of one example showed `text`, `retrieval_text` and `token_end` all genuinely differ between the two occurrences — not an exact repeated line. Cross-checked against `alzheimer_corpus/logs/quality_control.log`: Stage 06 (chunking) ran twice (an 18-chunk dev smoke test on 09-11 with the wrong tokenizer, unrelated; the real 4,377,041-chunk run on 09-18 with the correct MedCPT tokenizer), and Stage 07 (claim classification) started 4 times on 09-18 before completing cleanly on the 4th attempt. Conclusion: a small number of source documents survived Stage 05's near-duplicate detection as two distinct renderings of the same paper sharing a `document_id`; Stage 06 then generated the same `chunk_id` (`document_id` + section + index) for the first chunk of a shared section in each — a real, bounded, explainable corpus-build artifact, not a pipeline failure and not something introduced by any change in this session. **Not fixed by touching the frozen `chunks.jsonl`** — the corpus stays exactly as built. Instead, `read_passages_with_snapshot()` gained an explicit `on_duplicate` policy: `"raise"` (default, unchanged behaviour — safe for anyone who hasn't diagnosed their duplicates) or `"keep_first"` (keeps the first occurrence in file order, the same tie-break rule `DenseIndex` already uses, and returns every drop — `chunk_id`, kept/dropped line numbers, a text preview — so nothing is silently discarded). `build_index.py` gained `--on-duplicate {raise,keep_first}` (default `raise`) and writes a full `duplicate_chunks.json` report next to the index when duplicates are kept-past (never during `--dry-run`, which still writes nothing). 5 new tests (unknown-policy rejection, keep-first behaviour and its report contents, no-duplicates-no-report). Verified end-to-end against a synthetic corpus with an injected duplicate reproducing the real case (same id, different text): default raises with the same message the student saw; `--dry-run --on-duplicate keep_first` reports the count but writes nothing; the real run writes the report. 510 tests pass; the same 1 pre-existing, unrelated corpus-fixture failure. |
| 2026-09-21f | **Proactive audit of the whole repository for the same class of bug (whole-file reads / silent long-running loops / memory that doubles at scale), done before asking the student to run the real `build_index.py` (non-dry-run) — found and fixed a second, more serious issue in the encoding step it was about to reach.** Audit scope: every `read_text()`/`readlines()`/`json.load()` call in `experiments/` and `alzheimer_corpus/scripts/` (`grep`, not sampling); the corpus-build stages (already executed against the real 4.3M-chunk corpus, per §2) all go through `alzheimer_corpus/scripts/_common.py::read_jsonl`/`write_jsonl`, which already stream one record at a time and were already found safe - only `experiments/retrieval/` (the *consumption* side, a separate, deliberately decoupled layer) had unsafe patterns, one fixed yesterday (2026-09-21d/e) and one found today. **Found**: `MedCPTEncoder.encode()` (`experiments/retrieval/encoders.py`) collected every batch's output vectors into a Python list, then `np.vstack`-ed them at the end - holding *two* copies of the full embedding matrix at peak. At the real corpus's scale (4,377,041 chunks x MedCPT's 768 dims), that matrix is ~12.8 GB in float32; two copies is ~26 GB, well past the documented hardware's 16 GB RAM (`status_and_decisions.md` §5) - a near-certain crash or severe swapping on the very next command the student was about to run, not a hypothetical. Fixed by writing each batch directly into a preallocated output array (one copy, ~12.8 GB, still large but survivable). The module docstring in `index.py` was also wrong about scale ("a few hundred thousand chunks") - corrected to the real, measured number (4,377,041) and the real memory figure, rather than left to mislead the next reader. **Also added**: an `on_progress` callback on `MedCPTEncoder.encode()` (mirroring yesterday's pattern for corpus reading), wired through `build_index()` and the CLI - encoding hundreds of thousands of batches on CPU can run for hours, and `build_index.py` now prints elapsed/estimated-remaining time per batch instead of running silently, plus an explicit `--device cuda` recommendation (MedCPT's article encoder is ~0.44 GB, fits the student's 4 GB RTX 2050 - unlike Llama-3-8B, which cannot). **Honesty note**: this specific fix could not be executed or verified in this environment - `torch` is not installed here (by design, so the test suite and offline tools work without it), so `MedCPTEncoder.encode()` itself has never run anywhere in this repository's history, including now. It was corrected by code review, not by running it; the student's own next real run against actual MedCPT is the first execution of this code path, and should be watched for correctness, not assumed. 507 tests pass (2 new: progress-forwarding to an encoder that accepts it, and unaffected behaviour for one that doesn't - both against a stub, since `MedCPTEncoder` itself is untestable without torch); the same 1 pre-existing, unrelated corpus-fixture failure. |
| 2026-09-21e | **Investigated "the dry run is taking too long" after the OSError fix; found the redundant double file-read was not actually the cause, fixed it anyway since it's real, and added progress output since silence was the actual complaint.** `build_index.py` called `read_passages()` then `snapshot_id()` separately, reading the corpus file twice. Measured before assuming: on a 300k-line/439 MB synthetic corpus, one pass took 7.78s and two passes took 7.75s — the OS page cache made the second read free, so the double read was not the bottleneck. Consolidated it anyway into one function, `read_passages_with_snapshot()` (single pass, computes the same SHA-256 snapshot id while parsing — proven identical to the old two-call result by `test_combined_reader_matches_the_standalone_snapshot_id`, since a streaming hash's digest depends only on byte sequence, not chunking) — a real simplification, just not a speed fix. **The actual, honest cause of a large real corpus taking minutes is Python-level JSON parsing of millions of lines, which this change does not eliminate** — scaled to 4,377,041 chunks at this measured rate, roughly 2 minutes on comparable hardware; slower on the student's machine is plausible without indicating a bug (a `Documents`-folder path that OneDrive syncs is a known, common cause of large per-file-read overhead on Windows and was flagged to the student as worth checking, unverifiable from this environment). What was actually fixed for the complaint: `read_passages_with_snapshot()` gained an `on_progress` callback (silent by default, so library/test use is unaffected), and `build_index.py` now prints progress every 200,000 lines with elapsed time — so a multi-minute run looks alive instead of indistinguishable from hung. Verified end-to-end against a 500k-line synthetic corpus (prints two progress lines, ~2s each, then reports counts). 6 new tests (hash-equivalence, single-read regression guard, duplicate-id detection, progress-callback firing, silent-by-default). 505 tests pass; the same 1 pre-existing, unrelated corpus-fixture failure. |
...
| 2026-09-21b | **123-question evaluation pool audited; repaired 82-question set produced alongside the original.** A reviewer flagged the pool as too diplomatic, repetitive, and poorly designed for evaluation. Every one of the 123 `status="validated"` candidates was inspected individually (question quality + answer quality) against the thesis's actual evaluability requirement — a clear, objectively-checkable claim relevant to hallucination, outdated knowledge, out-of-context behaviour, or factual correctness — not against medical correctness alone. **Duplicate detection** used the Cochrane review ID (`CDxxxxxx`) embedded in `reference_locator`, grouped across `.pub` version history: 21 clusters, 53/123 questions (43%) involved. Each cluster's most recent version was kept (accurate current verdict); earlier versions were merged away — this simultaneously resolves the "many repeated questions" complaint and preserves the real, verified `temporal_candidate` signal (`.pub2`+ citations) rather than inventing new temporal labels. **Diplomatic/unusable answers**: per the verbatim-extraction-only provenance rule already in force in this repository, answer *text* cannot be reworded — doing so would fabricate wording not present in the cited source — so items whose reference answer contained no checkable claim at all (boilerplate methodology text, bare study counts, no verdict) were removed rather than rewritten; this is recorded as an explicit limitation, not silently worked around. **Disposition: 82 KEEP, 31 MERGE (duplicate resolution), 10 REMOVE (no checkable claim) — 0 answer text changed** (2 formatting-artifact IDs were moot: both were already the earlier half of a merged cluster). Original `experiments/questions/candidates.jsonl` and `review.csv` are untouched (confirmed via `git status`); the repaired set lives separately at `experiments/questions/repaired/` (`candidates.jsonl`, `review.csv` — the latter generated by the existing, unmodified `export_review.py` tool, not hand-written). Provenance kept via `experiments/questions/audit_table.csv` (per-question original ID → final disposition → reason) and the full narrative in `experiments/questions/audit_report.md`. **Validated**: `export_review.py --pool-dir experiments/questions/repaired` ran clean (82/82 reviewable, 0 missing provenance); all 82 repaired records directly constructed as `EvaluationQuestion` with 0 validation failures; full test suite unaffected (472 tests, same 2 pre-existing unrelated failures). Coverage of the repaired 82: 82/82 hallucination-evaluable, 48/82 outdated-knowledge-evaluable (`temporal_candidate=True`), 82/82 out-of-context-evaluable, 82/82 factual-correctness-evaluable (72 Cochrane-sourced, 10 MedQuAD-sourced). 82 is below the original 123 and below the informal ~100 target; no replacement items were manufactured to pad the count, since doing so would require fabricating new sourced evidence — this gap is recorded as an open item rather than closed artificially. Human review of the repaired pool (`review.csv` → `review_decision`) has not yet occurred. Also on 2026-09-21b: **Question review actually completed by the student, directly on main: all 123 questions decided.** Recorded in `experiments/questions/review.csv`: **90 ACCEPT, 23 REVISE, 10 REJECT, 0 HOLD** — real source-checked judgement (Cochrane Library / PubMed), which this pipeline's own automated passes could never perform, since both are network-egress-blocked in this build environment (confirmed by direct test). This supersedes and discards an earlier, explicitly-invalidated automated substitute that had been proposed on a feature branch; that branch's PR was closed without merging once the real review landed, specifically so it could not overwrite it. §3.1's "Human question review" blocker is closed; §3.2 step 1 marked done. |
