...
def resolve_pilot_size(configured: Any, pool: str) -> Optional[int]:
    """Per-pool candidate limit, or None for "use everything".

    A single shared number was a trap: the primary evaluation is meant to use
    the FULL eligible external pool (ledger D-23), so a pilot limit left in
    the config would silently take a seeded sample of it - and Check A, the
    one measurement most likely to redirect the thesis, would then be
    computed on a fraction of the items. Sizing is therefore declared per
    pool and an int that applies to both must be written out as such.
    """
    ...
    if isinstance(configured, dict):
        return configured.get(pool)

    if isinstance(configured, int):
        return configured

    raise ValueError(
        "pilot.size must be null, an integer, or a mapping of pool name to "
        f"size; got {type(configured).__name__}."
    )
...
def run(
    *,
    ...
    # Recorded in every pair's provenance, so it is part of the frozen file
    # and therefore part of its hash. Taken from the config when given, so a
    # frozen evaluation set can be regenerated byte-identically later; only
    # an unconfigured run stamps today's date.
    configured_extraction_date = config.get("extraction_date")
    extraction_date = str(configured_extraction_date or _utc_today())
    ...
    if not input_path.exists():
        raise FileNotFoundError(
            ...
            + (
                ...
                "created: see docs/research_experimental_specification.md "
                "§14 for the input contract and the outstanding "
                "dependency."
    ...
    limit = resolve_pilot_size(pilot_cfg.get("size"), pool)
    pilot = sample_pilot(candidates, size=limit, seed=seed)
    ...
    manifest = {
        ...
        "extraction_date": extraction_date,
        "extraction_date_source": (
            "config" if configured_extraction_date else "run_date"
        ),
        "candidates_enumerated": len(candidates),
        "pilot_size_requested": limit,
        "pilot_truncated": limit is not None and limit < len(candidates),
