...
sys.path.insert(0, str(ROOT / "alzheimer_corpus" / "scripts" / "classification"))
...
class ConfigIntegrityTest(unittest.TestCase):
    def test_source_tiers_define_no_ordering(self):
        ...
        self.assertIsNone(clf.corpus_cfg["source_tiers"]["ordering"])

    def test_licensing_fails_closed(self):
        clf = ADRelevanceClassifier()
        self.assertFalse(clf.corpus_cfg["licensing"]["default_redistribution_allowed"])
